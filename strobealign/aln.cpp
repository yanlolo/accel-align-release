...
struct ScoredAlignmentPair {
    double score;
  StrobAlignment alignment1;
  StrobAlignment alignment2;
};

static inline StrobAlignment extend_seed(
...
static inline void align_SE(
    ...
    std::vector<StrobAlignment> alignments;
    int tries = 0;
    Nam n_max = nams[0];
  ...
  StrobAlignment best_alignment;
    ...
    std::sort(alignments.begin(), alignments.end(),
        [](const StrobAlignment& a, const StrobAlignment& b) -> bool {
            return a.score > b.score;
        }
    );
...
static inline std::vector<ScoredAlignmentPair> get_best_scoring_pairs(
    const std::vector<StrobAlignment>& alignments1,
    const std::vector<StrobAlignment>& alignments2,
    float mu,
    float sigma
) {
...
static inline StrobAlignment rescue_mate(
    const Aligner& aligner,
    const Nam &nam,
    const References& references,
    const Read& read,
...
void rescue_read(
    ...
    std::vector<StrobAlignment> alignments1;
    std::vector<StrobAlignment> alignments2;
    for (auto& nam : nams1) {
      ...
      StrobAlignment a2 = rescue_mate(aligner, nam, references, read2, mu, sigma, k);
        details[1].mate_rescue += !a2.is_unaligned;
        alignments2.emplace_back(a2);

        tries++;
    }
    std::sort(alignments1.begin(), alignments1.end(), by_score<StrobAlignment>);
    std::sort(alignments2.begin(), alignments2.end(), by_score<StrobAlignment>);
    ...
    if (max_secondary == 0) {
        auto best_aln_pair = high_scores[0];
      StrobAlignment alignment1 = best_aln_pair.alignment1;
      StrobAlignment alignment2 = best_aln_pair.alignment2;
        if (swap_r1r2) {
        ...
        for (size_t i = 0; i < max_out; ++i) {
          ...
          StrobAlignment alignment1 = aln_pair.alignment1;
          StrobAlignment alignment2 = aln_pair.alignment2;
...
inline void align_PE(
    ...
    robin_hood::unordered_map<int,StrobAlignment> is_aligned1;
    robin_hood::unordered_map<int,StrobAlignment> is_aligned2;

    // These keep track of the alignments that would be best if we treated
    // the paired-end read as two single-end reads.
  StrobAlignment a1_indv_max, a2_indv_max;
    {
        auto n1_max = nams1[0];
...
void align_PE_read(
    ...
    // Find NAMs
    Timer nam_timer;
    auto [nonrepetitive_fraction1, nams1] = find_nams(query_randstrobes1, index, false);
    auto [nonrepetitive_fraction2, nams2] = find_nams(query_randstrobes2, index, false);
    statistics.tot_find_nams += nam_timer.duration();
    if (map_param.rescue_level > 1) {
        Timer rescue_timer;
        if (nams1.empty() || nonrepetitive_fraction1 < 0.7) {
            nams1 = find_nams_rescue(query_randstrobes1, index, map_param.rescue_cutoff, false);
            details[0].nam_rescue = true;
        }

        if (nams2.empty() || nonrepetitive_fraction2 < 0.7) {
            nams2 = find_nams_rescue(query_randstrobes2, index, map_param.rescue_cutoff, false);
...
void align_SE_read(
    ...
    // Find NAMs
    Timer nam_timer;
    auto [nonrepetitive_fraction, nams] = find_nams(query_randstrobes, index, false);
    statistics.tot_find_nams += nam_timer.duration();
    ...
    if (map_param.rescue_level > 1) {
        ...
        if (nams.empty() || nonrepetitive_fraction < 0.7) {
            ...
            nams = find_nams_rescue(query_randstrobes, index, map_param.rescue_cutoff, false);
        }
        statistics.tot_time_rescue += rescue_timer.duration();
    }
