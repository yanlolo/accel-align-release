...
int Embedding::hamdist(const char *r, const char *ref, int rlen){
  int cnt = 0;
  ...
  for (unsigned i = 0; i < candidate_regions.size(); ++i) {
    ...
    for (unsigned strid = 0; strid < NUM_STR; ++strid) {
      if (best_threshold <= 1 && next_threshold <= 1) {
        ...
        if (nmismatch > 2)
          nmismatch = cgk2_unmatched(r, ptr_ref + region.rs, region.matched_intervals, R.rlen, R.kmer_step, next_threshold, strid);
      }
