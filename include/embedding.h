...
class Embedding {
 public:
  ...
  void embed_unmatch_iter(vector<Region> &candidate_regions, const char *ptr_ref, Read &R,
                          int &best_threshold, int &next_threshold, unsigned &best_idx, unsigned &next_idx);
  void embed_unmatch(vector<Region> &candidate_regions, const char *ptr_ref, Read &R, bool flag_f1[]);
