### 1. Seeding

#### 1.1 Compile
```
make
make accindex_stats
```

#### 1.2 Seeding types

##### 1.2.1 hash-based
```
code_dir=/home/yiqing/code/accel-align-release/
ref=/home/yiqing/data/fsva-hg37/hg37.fna
mod=536870879
xxh=0

## index stats --> all mod/hash
$code_dir/accindex_stats -l 21 -h ${mod} -x ${xxh} $ref

sh utilities/seed-stats.sh


```

##### 1.2.2 minimizer-based
```
code_dir=/home/yiqing/code/accel-align-release/
ref=/home/yiqing/data/fsva-hg37/hg37.fna
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output/
nthreads=12
rlen=100

## index stats
$code_dir/accindex_stats -m -k 15 -w 10 $ref
python $code_dir/utilities/seed-stats.py ${mod}-${xxh} $out_dir

## index
$code_dir/accindex -m -k 15 -w 10 $ref

## align
time $code_dir/accalign -m -t $nthreads -o $out_dir/mini.sam $ref \
$in_dir/sv-1m-${rlen}-r1.fastq $in_dir/sv-1m-${rlen}-r2.fastq

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-${rlen}-pe-align.sam $out_dir/mini.sam $out_dir/hash.csv

```


##### 1.2.3 strobe-based
```
code_dir=/home/yiqing/code/accel-align-release
ref=/home/yiqing/data/fsva-hg37/hg37.fna
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output/
nthreads=12
rlen=100

## index-stats
$code_dir/accindex_stats --strobe-mode --create-index $ref -r $rlen

## index
$code_dir/accindex --strobe-mode --create-index $ref -r $rlen

##align SE
time $code_dir/accalign --strobe-mode -I H -t $nthreads $ref \
$in_dir/sv-1m-${rlen}-r.fastq > $out_dir/strobemer.sam

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-${rlen}-se-align.sam $out_dir/strobemer.sam $out_dir/hash.csv


##align PE
time $code_dir/accalign --strobe-mode -I H -t $nthreads $ref \
$in_dir/sv-1m-${rlen}-r1.fastq $in_dir/sv-1m-${rlen}-r2.fastq > $out_dir/strobemer.sam

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-${rlen}-pe-align.sam $out_dir/strobemer.sam $out_dir/hash.csv
```

### 2. Index Lookup

#### 2.1 RMI

```
## Index
code_dir=/home/yiqing/code/accel-align-release
ref=/home/yiqing/data/fsva-hg37/hg37.fna
bash $code_dir/rmi.sh -l 32 $ref
## output: 
#### /home/yiqing/data/fsva-hg37/hg37_index32
#### /home/yiqing/data/fsva-hg37/hg37_index32/keys_uint64: (data.key, i) --> i is accumulated because it's the the number of seeds lower than this value
#### /home/yiqing/data/fsva-hg37/hg37_index32/pos_uint32: (data.pos)
#### /home/yiqing/data/fsva-hg37/hg37_index32/rmi_data/rmi_L1_PARAMETERS: learned statistic

## Note1: [rmi.sh] key_gen output already exists. Do you want to execute the command anyway? [y/N] --> y
## Note2: [rmi.sh] which index would you like to train? [1-10] 1

## align
code_dir=/home/yiqing/code/accel-align-release
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output
ref=/home/yiqing/data/fsva-hg37/hg37.fna
nthreads=12

## PE
in_dir=/home/yiqing/yan/input/simulate/1m/
$code_dir/accalign -l 32 -R -t $nthreads -o $out_dir/rmi.sam $ref \
$in_dir/sv-1m-100-r1.fastq $in_dir/sv-1m-100-r2.fastq

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-100-pe-align.sam $out_dir/rmi.sam $out_dir/rmi.csv  


```

#### 2.1.1 RMI-strobe

```
## Index
code_dir=/home/yiqing/code/accel-align-release
ref=/home/yiqing/data/fsva-hg37/hg37.fna
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output/
nthreads=12
rlen=100

bash $code_dir/rmi.sh -l 32 $ref

## Note1: manuelly change rmi.sh line 103 to run ""${BASE_DIR}/accindex" --strobe-mode --create-index $ref_name -r 100" 
## Note 2: '-l 32' is to generate output as the following scheme 
## Output  --> keyv, posv, learnt index
#### /home/yiqing/data/fsva-hg37/hg37_index32
#### /home/yiqing/data/fsva-hg37/hg37_index32/keys_uint64: (data.key, i) --> i is accumulated because it's the the number of seeds lower than this value
#### /home/yiqing/data/fsva-hg37/hg37_index32/pos_uint32: (data.pos)
#### /home/yiqing/data/fsva-hg37/hg37_index32/rmi_data/rmi_L1_PARAMETERS: learned statistic
## Output  --> strobemer
#### /home/yiqing/data/fsva-hg37/hg37.fna.r100.sti


## Align
## change accalign.cpp:2142 ->  IndexType index_type = IndexType::RMI_IDX; 

code_dir=/home/yiqing/code/accel-align-release
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output
ref=/home/yiqing/data/fsva-hg37/hg37.fna
nthreads=12

## PE
in_dir=/home/yiqing/yan/input/simulate/1m/
time $code_dir/accalign --strobe-mode -I R -t $nthreads -o $out_dir/rmi.sam $ref \
$in_dir/sv-1m-100-r1.fastq $in_dir/sv-1m-100-r2.fastq

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-100-pe-align.sam $out_dir/rmi.sam $out_dir/rmi.csv  

```

#### 2.2 Binary

```
## Index (make key_gen -> generated by RMI already)
code_dir=/home/yiqing/code/accel-align-release
ref=/home/yiqing/data/fsva-hg37/hg37.fna
$code_dir/key_gen -l 32 $ref

## align
code_dir=/home/yiqing/code/accel-align-release
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output
ref=/home/yiqing/data/fsva-hg37/hg37.fna
nthreads=4

time $code_dir/accalign -l 32 -B -t $nthreads -o $out_dir/binary.sam $ref \
$in_dir/sv-1m-100-r1.fastq $in_dir/sv-1m-100-r2.fastq

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-100-pe-align.sam $out_dir/binary.sam $out_dir/binary.csv  

```


#### 2.3 Hash

```
##Index
###### -h {prime, lprime, 2^29, 2^32} ###### 
## mod by {1073741651, 2861333663, 2^29-1, 2^32-1}
###### -x {0, 32, 64} ###### 
## XXH not used, XXH32, XXH64.

## output  /home/yiqing/data/fsva-hg37/hg37.fna.hash32 (kmer len)
code_dir=/home/yiqing/code/accel-align-release
ref=/home/yiqing/data/fsva-hg37/hg37.fna
$code_dir/accindex -l 32 -h 2^29 -x 0 $ref


## Align
code_dir=/home/yiqing/code/accel-align-release
in_dir=/home/yiqing/yan/input/simulate/1m/
out_dir=/home/yiqing/yan/output
ref=/home/yiqing/data/fsva-hg37/hg37.fna
nthreads=4

$code_dir/accalign -l 32 -H -t $nthreads -o $out_dir/hash.sam $ref \
$in_dir/sv-1m-100-r1.fastq $in_dir/sv-1m-100-r2.fastq

## accuracy check
python $code_dir/utilities/match.py $in_dir/sv-1m-100-pe-align.sam $out_dir/hash.sam $out_dir/hash.csv  

```

